...
void gstreamer_sink_base::pushImage(TS<SyncedMemory> img, cv::cuda::Stream& stream)
{
    ...
    auto curTime = mo::getCurrentTime();
    // TODO rework with corrected time
...
void gstreamer_sink_base::pushImage(SyncedMemory img, cv::cuda::Stream& stream, mo::OptionalTime_t ts)
{
    MO_LOG_EVERY_N(debug, 100) << "Pushing image onto pipeline";
    std::chrono::system_clock::duration delta = std::chrono::milliseconds(33);
    if (ts)
    {
        if (_prev_time)
        {
    ...
    MO_LOG(trace) << "Estimated frame time: " << delta << " ms";
    ...
    if (_feed_enabled)
    {
        ...
        if (img.getSyncState() < img.DEVICE_UPDATED)
        {
            ...
            GST_BUFFER_DURATION(buffer) = std::chrono::duration_cast<std::chrono::nanoseconds>(delta).count();
