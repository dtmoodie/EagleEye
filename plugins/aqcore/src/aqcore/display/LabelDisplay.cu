...
template <class T>
void __global__ apply_lut_kernel(const cv::Vec3b* lut,
                                 const cv::cuda::PtrStepSz<T> in,
                                 cv::cuda::PtrStepSz<cv::Vec3b> out,
                                 int num_labels)
{
...
namespace aq
{
                       ...
                       cv::cuda::Stream& stream)
    {
        ...
        dim3 block(32, 8);
        dim3 grid(cv::cuda::device::divUp(input_8U.cols, block.x), cv::cuda::device::divUp(input_8U.rows, block.y));
        if (input_8U.depth() == CV_8U)
            apply_lut_kernel<uchar><<<grid, block, 0, cv::cuda::StreamAccessor::getStream(stream)>>>(
                colormap.ptr<cv::Vec3b>(), input_8U, output_8UC3, colormap.cols);
        else
            apply_lut_kernel<int><<<grid, block, 0, cv::cuda::StreamAccessor::getStream(stream)>>>(
                colormap.ptr<cv::Vec3b>(), input_8U, output_8UC3, colormap.cols);
    }
} // namespace aq
