...
bool FrameLimiter::processImpl()
{
    ...
    boost::posix_time::time_duration delta(current_time - last_time);

    int goal_time = int(1000.0 / desired_framerate);
    if (delta.total_milliseconds() > goal_time)
    {
        last_time = current_time;
        output_param.updateData(*input, mo::tag::_param = input_param);
    }
...
{
    if (data_type_param.modified() || channels_param.modified() || width_param.modified() || height_param.modified() ||
        fill_param.modified())
    {
        cv::cuda::GpuMat mat;
...
bool Resize::processImpl()
{
    if (input && !input->empty())
    {
        if (input->getSyncState() < SyncedMemory::DEVICE_UPDATED)
        {
            cv::Mat resized;
            cv::resize(
                input->getMat(stream()), resized, cv::Size(width, height), 0.0, 0.0, interpolation_method.getValue());
        ...
        else
        {
            ...
            cv::cuda::resize(input->getGpuMat(stream()),
                             resized,
...
bool Subtract::processImpl()
{
    if (input->getSyncState() < SyncedMemory::DEVICE_UPDATED)
    {
        cv::subtract(input->getMat(stream()),
                     value,
                     output.getMatMutable(stream()),
                     mask ? mask->getMat(stream()) : cv::noArray(),
                     dtype.getValue());
    }
    else
    {
        cv::cuda::subtract(input->getGpuMat(stream()),
                           value,
                           output.getGpuMatMutable(stream()),
                           mask ? mask->getGpuMat(stream()) : cv::noArray(),
                           dtype.getValue(),
                           stream());
    }
    return true;
