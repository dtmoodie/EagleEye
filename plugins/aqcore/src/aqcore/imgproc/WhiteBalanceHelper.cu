...
#include "Aquila/utilities/cuda/cuda_sorting.hpp"
...
struct Saturate
{
    ...
    typename cv::cudev::MakeVec<U, 3>::type operator()(const typename cv::cudev::MakeVec<T, 3>::type& vec_in)
    {
...
void __global__ transform_kernel(const cv::cuda::PtrStepSz<typename cv::cudev::MakeVec<T1, 3>::type> in,
                                 ...
                                 cv::cuda::PtrStepSz<float> saturate,
...
template <class T1, class T2>
void transform(const cv::cuda::GpuMat& in,
               cv::cuda::GpuMat& out,
               const cv::cuda::PtrStepSzf& saturate,
               float dyn_range,
               cudaStream_t stream)
{
    dim3 block(32, 8);
    dim3 grid(cv::cuda::device::divUp(in.cols, block.x), cv::cuda::device::divUp(in.rows, block.y));
...
namespace aq
{
                           ...
                           cv::cuda::Stream& stream_)
    {
        ...
        for (int i = 0; i < sample_regions.size(); ++i)
        {
            ...
            for (int j = 0; j < 3; ++j)
            {
                ...
                if (input.depth() == CV_8U)
                {
                    ...
                    accum_kernel<uchar><<<1, 1, 0, stream>>>(flat.data + lower_offset, accum_ptr + 2 * j);

                    accum_kernel<uchar><<<1, 1, 0, stream>>>(flat.data + upper_offset, accum_ptr + 2 * j + 1);
                }
                else if (input.depth() == CV_16U)
                {
                    ...
                    accum_kernel<ushort><<<1, 1, 0, stream>>>(reinterpret_cast<ushort*>(flat.data) + lower_offset,
                                                              accumulation.ptr<float>(j));

                    accum_kernel<ushort><<<1, 1, 0, stream>>>(reinterpret_cast<ushort*>(flat.data) + upper_offset,
        ...
        typedef void (*func_t)(const cv::cuda::GpuMat& in,
                               cv::cuda::GpuMat& out,
                               const cv::cuda::PtrStepSzf& saturate,
                               float dyn_range,
                               cudaStream_t stream);
        static const func_t funcs[3][3] = {{transform<uchar, uchar>, 0, transform<ushort, uchar>},
                                           {0, 0, 0},
    ...
    void colorCorrect(cv::cuda::GpuMat& in_out, const cv::cuda::GpuMat& color_matrix, cv::cuda::Stream& stream_)
    {
        ...
        dim3 block(32, 8);
        dim3 grid(cv::cuda::device::divUp(in_out.cols, block.x), cv::cuda::device::divUp(in_out.rows, block.y));
        cudaStream_t stream = cv::cuda::StreamAccessor::getStream(stream_);
        switch (in_out.depth())
        {
        case CV_8U:
            return color_correct_kernel<uchar><<<grid, block, 0, stream>>>(in_out, color_matrix);
        case CV_16U:
            return color_correct_kernel<ushort><<<grid, block, 0, stream>>>(in_out, color_matrix);
        case CV_32F:
            return color_correct_kernel<float><<<grid, block, 0, stream>>>(in_out, color_matrix);
        default:
            return;
        }
    }
