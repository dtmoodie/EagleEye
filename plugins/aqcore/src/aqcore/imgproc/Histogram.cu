...
template<typename T>
__host__ __device__ inline T* binary_search_approx(T *const begin, T *const end, T value)
{
    T* q = begin;
    if(begin >= end)
    {
                         ...
                         cv::cuda::Stream& stream)
{
    ...
    if(bins.empty() && in.depth() != CV_8U)
    {
        ...
        thrust::sequence(
                    ...
                    thrust::system::cuda::par.on(cv::cuda::StreamAccessor::getStream(stream)),
#endif
                    ptr, ptr + size, min, step);
    }
