...
namespace aqcore
{
    bool MorphologyFilter::processImpl(aq::CVStream& stream)
    {
        auto cvstream = stream.getCVStream();
        if (structuring_element_type_param.getModified() || morphology_type_param.getModified() ||
            anchor_point_param.getModified() || iterations_param.getModified() || filter == nullptr)
        {
            cv::Mat structuring_element =
                cv::getStructuringElement(structuring_element_type.current_selection,
                                          ...
                                          anchor_point);
            cv::Mat tmp = structuring_element;
            structuring_element_param.setValue(std::move(structuring_element));

            filter = cv::cuda::createMorphologyFilter(morphology_type.current_selection,
                                                      input->getMat(&stream).type(),
                                                      structuring_element,
                                                      anchor_point,
                                                      iterations);

            structuring_element_size_param.setModified(false);
            morphology_type_param.setModified(false);
            anchor_point_param.setModified(false);
            iterations_param.setModified(false);
        }
        cv::cuda::GpuMat out;
        filter->apply(input->getGpuMat(&stream), out, stream.getCVStream());
        this->output.publish(out, mo::tags::param = &input_param, mo::tags::stream = &stream);
        return true;
    }
    ...
    */
    ...
    bool FindContours::processImpl(mo::IAsyncStream& stream)
    {
        cv::Mat h_mat = input->getMat(&stream);
        ...
        this->contours.publish(std::move(contours), mo::tags::param = &input_param, mo::tags::stream = &stream);
        this->hierarchy.publish(std::move(hierarchy), mo::tags::param = &input_param, mo::tags::stream = &stream);
    ...
    *thresholdParam->Data(); }), areaPtr->end());
    ...
    bool ContourBoundingBox::processImpl(mo::IAsyncStream& stream)
    {
        ...
        if (merge_contours)
        {
            ...
            {
                // Find overlapping boxes and merge them
            }
        }

        this->output.publish(boxes, mo::tags::param = &this->contours_param, mo::tags::stream = &stream);
        return true;
    }
    ...
    bool DrawContours::processImpl(mo::IAsyncStream& stream)
    {
        const cv::Mat& image = this->image->getMat(&stream);
        cv::Mat output_image;
        image.copyTo(output_image);
        int largest_idx = -1;
        if (LargestSize == draw_mode.getValue() || LargestArea == draw_mode.getValue())
        {
            double largest_count = 0;
            for (int i = 0; i < contours->size(); ++i)
            {
                double size;
                if (LargestSize == draw_mode.getValue())
                {
        ...
        cv::drawContours(output_image, *contours, largest_idx, cv::Scalar(0, 255, 0));
        output.publish(output_image, mo::tags::stream = &stream, mo::tags::param = &image_param);
        return true;
    }
} // namespace aqcore
