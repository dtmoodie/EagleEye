...
bool GoodFeaturesToTrack::processImpl()
{
    ...
    if (!detector || max_corners_param.modified() || quality_level_param.modified() || min_distance_param.modified() ||
        block_size_param.modified() || use_harris_param.modified() /* || harris_K_param.modified()*/)
    {
        detector = cv::cuda::createGoodFeaturesToTrackDetector(
            input->getDepth(), max_corners, quality_level, min_distance, block_size, use_harris);
...
{
    if (threshold_param.modified() || use_nonmax_suppression_param.modified() || fast_type_param.modified() ||
        max_points_param.modified() || detector == nullptr)
    {
        detector =
            cv::cuda::FastFeatureDetector::create(threshold, use_nonmax_suppression, fast_type.getValue(), max_points);
    }
    cv::cuda::GpuMat keypoints;
    if (mask)
    {
...
{
    if (num_features_param.modified() || scale_factor_param.modified() || num_levels_param.modified() ||
        edge_threshold_param.modified() || first_level_param.modified() || WTA_K_param.modified() ||
        score_type_param.modified() || patch_size_param.modified() || fast_threshold_param.modified() ||
        blur_for_descriptor_param.modified() || detector == nullptr)
    {
    ...
    if (mask)
    {
        detector->detectAndComputeAsync(
            input->getGpuMat(stream()), mask->getGpuMat(stream()), keypoints, descriptors, false, stream());
    }
    else
    {
        detector->detectAndComputeAsync(
            input->getGpuMat(stream()), cv::noArray(), keypoints, descriptors, false, stream());
    }
    keypoints_param.updateData(keypoints, input_param.getTimestamp(), _ctx.get());
...
{
    if (block_size_param.modified() || sobel_aperature_size_param.modified() ||
        harris_free_parameter_param.modified() || detector == nullptr)
    {
        detector =
            cv::cuda::createHarrisCorner(input->getType(), block_size, sobel_aperature_size, harris_free_parameter);
    }
...
bool CornerMinEigenValue::processImpl()
{
    if (block_size_param.modified() || sobel_aperature_size_param.modified() ||
        harris_free_parameter_param.modified() || detector == nullptr)
    {
        detector = cv::cuda::createMinEigenValCorner(
            input->getType(), block_size, sobel_aperature_size, harris_free_parameter);
    }
    cv::cuda::GpuMat score;
    detector->compute(input->getGpuMat(stream()), score, stream());
    score_param.updateData(score, input_param.getTimestamp(), _ctx.get());
