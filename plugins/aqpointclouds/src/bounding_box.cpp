...
namespace detail
{
    void contains_kernel(
        const cv::cuda::GpuMat& points, cv::cuda::GpuMat& mask, cv::Mat& T, cv::Scalar& size, cv::cuda::Stream& stream);
}

bounding_box::bounding_box(cv::Mat transform, cv::Scalar size) : _size(size)
{
