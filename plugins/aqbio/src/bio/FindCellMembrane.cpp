...
namespace aqbio
{
    ...
    float gaussianDist(const float sigma, const float x) { return gaussianDist(sigma, 0.0F, x); }
    ...
    {
        Eigen::ParametrizedLine<float, 2> ray = Eigen::ParametrizedLine<float, 2>::Through(center, {pt.x, pt.y});
    ...
    void FindCellMembrane::mouseDrag(
        std::string /*window_name*/, cv::Point start, cv::Point end, int /*flags*/, cv::Mat /*img*/)
    {
        ...
        setModified();
        sig_update();
        user_update_param.setValue(true);
    }
                                                               ...
                                                               const aq::Circle<float>& circle)
    {
        ...
        for (size_t i = 0; i < inner.size(); ++i)
        {
            ...
            for (float x = circle.radius * outer_pad; x > circle.radius * 0.9f; x -= 0.5f)
            {
                ...
                {
                    float d = abs(
                        ray.direction().dot(Eigen::Vector2f(dx.at<short>(pt[1], pt[0]), dy.at<short>(pt[1], pt[0]))));
                    Dacc(d);
                    D.push_back(d);
    ...
    void FindCellMembrane::reweightEnergy(cv::Mat_<float>& energy)
    {
        ...
        // for each line weight the observations based on passing a threshold, starting from the outside and moving
        // inwards.
    ...
    bool FindCellMembrane::snakePoints(const cv::Mat& img,
                                       std::vector<cv::Point>& points,
                                       const std::vector<float>& point_weight)
    {
