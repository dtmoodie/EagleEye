...
QCPLayer::~QCPLayer()
{
    ...
    if (mParentPlot->currentLayer() == this)
        qDebug() << Q_FUNC_INFO << "The parent plot's mCurrentLayer will be a dangling pointer. Should have been set "
                                   "to a valid layer or 0 beforehand.";
}
...
void QCPLayoutElement::update(UpdatePhase phase)
{
    if (phase == upMargins)
    {
        ...
        {
            // set the margins of this layout element according to automatic margin calculation, either directly or via
            // a margin group:
            QMargins newMargins = mMargins;
            QList<QCP::MarginSide> allMarginSides = QList<QCP::MarginSide>() << QCP::msLeft << QCP::msRight
                                        ...
                                        int totalSize) const
{
               ...
               sectionCount * 2) // the iteration check ist just a failsafe in case something really strange happens
    {
                   ...
                   sectionCount * 2) // the iteration check ist just a failsafe in case something really strange happens
        {
            ...
            // sections so far that the found section
            ...
            if (nextMax < nextMaxLimit) // next maximum is actually hit, move forward to that point and fix the size of
                                        // that section
            {
                for (int i = 0; i < unfinishedSections.size(); ++i)
                {
        ...
        if (foundMinimumViolation)
        {
            ...
            // reset all section sizes to zero that are in unfinished sections (all others have been set to their
            // minimum):
            for (int i = 0; i < unfinishedSections.size(); ++i)
...
void QCPLayoutGrid::simplify()
{
    ...
    for (int row = rowCount() - 1; row >= 0; --row)
    {
        ...
        if (!hasElements)
        {
            ...
            if (mElements.isEmpty()) // removed last element, also remove stretch factor (wouldn't happen below because
                                     // also columnCount changed to 0 now)
...
void QCPLineEnding::draw(QCPPainter* painter, const QVector2D& pos, const QVector2D& dir) const
{
    ...
    {
        QPointF points[3] = {
            pos.toPointF(), (pos - lengthVec + widthVec).toPointF(), (pos - lengthVec - widthVec).toPointF()};
    ...
    {
        QPointF points[3] = {
            (pos - lengthVec + widthVec).toPointF(), pos.toPointF(), (pos - lengthVec - widthVec).toPointF()};
        painter->setPen(miterPen);
    ...
    case esSkewedBar:
    {
        ...
        {
            // if drawing with cosmetic pen (perfectly thin stroke, happens only in vector exports), draw bar exactly on
            // tip of line
        ...
        {
            // if drawing with thick (non-cosmetic) pen, shift bar a little in line direction to prevent line from
            // sticking through bar slightly
...
void QCPAxis::rescale(bool onlyVisiblePlottables)
{
    ...
    if (haveRange)
    {
        ...
        {
            double center = (newRange.lower + newRange.upper) * 0.5; // upper and lower should be equal anyway, but just
                                                                     // to make sure, incase validRange returned false
                                                                     // for other reason
            if (mScaleType == stLinear)
            {
...
#else
                mTickVectorLabels[i] = mParentPlot->locale().toString(
                    QDateTime::fromMSecsSinceEpoch(mTickVector.at(i) * 1000).toTimeSpec(mDateTimeSpec),
                    mDateTimeFormat);
#endif
            }
        }
    ...
    if (mScaleType == stLinear)
    {
        if (mAutoTickStep)
        {
            ...
            mTickStep = mRange.size() / (double)(mAutoTickCount + 1e-10); // mAutoTickCount ticks on average, the small
                                                                          // addition is to prevent jitter on exact
                                                                          // integers
...
void QCPAxisPainterPrivate::draw(QCPPainter* painter)
{
    ...
    if (type == QCPAxis::atLeft)
    {
        mAxisSelectionBox.setCoords(
            origin.x() - selAxisOutSize, axisRect.top(), origin.x() + selAxisInSize, axisRect.bottom());
        ...
        mLabelSelectionBox.setCoords(
            origin.x() - selLabelOffset - selLabelSize, axisRect.top(), origin.x() - selLabelOffset, axisRect.bottom());
    }
    else if (type == QCPAxis::atRight)
    {
        mAxisSelectionBox.setCoords(
            origin.x() - selAxisInSize, axisRect.top(), origin.x() + selAxisOutSize, axisRect.bottom());
        mTickLabelsSelectionBox.setCoords(origin.x() + selTickLabelOffset + selTickLabelSize,
                                          axisRect.top(),
                                          origin.x() + selTickLabelOffset,
                                          axisRect.bottom());
    ...
    {
        mAxisSelectionBox.setCoords(
            axisRect.left(), origin.y() - selAxisOutSize, axisRect.right(), origin.y() + selAxisInSize);
        mTickLabelsSelectionBox.setCoords(axisRect.left(),
                                          origin.y() - selTickLabelOffset - selTickLabelSize,
                                          axisRect.right(),
                                          origin.y() - selTickLabelOffset);
        mLabelSelectionBox.setCoords(
            axisRect.left(), origin.y() - selLabelOffset - selLabelSize, axisRect.right(), origin.y() - selLabelOffset);
    }
    else if (type == QCPAxis::atBottom)
    {
        mAxisSelectionBox.setCoords(
            axisRect.left(), origin.y() - selAxisInSize, axisRect.right(), origin.y() + selAxisOutSize);
        mTickLabelsSelectionBox.setCoords(axisRect.left(),
                                          origin.y() + selTickLabelOffset + selTickLabelSize,
                                          axisRect.right(),
                                          origin.y() + selTickLabelOffset);
                                                                             ...
                                                                             const QString& text) const
{
    ...
    if (result.baseFont.pointSizeF() >
        ...
        result.baseFont.setPointSizeF(result.baseFont.pointSizeF() + 0.05); // QFontMetrics.boundingRect has a bug for
                                                                            // exact point sizes that make the results
                                                                            // oscillate due to internal rounding
    if (useBeautifulPowers)
    {
        ...
        result.totalBounds = result.baseBounds.adjusted(
            0, 0, result.expBounds.width() + 2, 0); // +2 consists of the 1 pixel spacing between base and exponent (see
...
/*! \class QCPAbstractPlottable
  ...
  coordinates to pixels in either the key or value dimension.
        Make sure to check whether the pointer is null before using it. If one of the axes is null, don't draw the
  plottable.</td>
  </tr><tr>
...
/*! \class QCPAbstractItem
  ...
  (e.g. arrows).</td></tr>
  <tr><td>QCPItemStraightLine</td><td>A straight line defined by a start and a direction point. Unlike QCPItemLine, the
  straight line is infinitely long and has no endings.</td></tr>
  <tr><td>QCPItemCurve</td><td>A curve defined by start, end and two intermediate control points. May have different
  ending styles on each side (e.g. arrows).</td></tr>
  ...
  <tr><td>QCPItemTracer</td><td>An item that can be attached to a QCPGraph and sticks to its data points, given a key
  coordinate.</td></tr>
  </table>
...
QCPAbstractPlottable* QCustomPlot::plottableAt(const QPointF& pos, bool onlySelectable) const
{
    ...
    foreach (QCPAbstractPlottable* plottable, mPlottables)
    {
        if (onlySelectable && !plottable->selectable()) // we could have also passed onlySelectable to the selectTest
...
QCPAbstractItem* QCustomPlot::itemAt(const QPointF& pos, bool onlySelectable) const
{
    ...
    foreach (QCPAbstractItem* item, mItems)
    {
        if (onlySelectable && !item->selectable()) // we could have also passed onlySelectable to the selectTest
...
void QCustomPlot::replot(QCustomPlot::RefreshPriority refreshPriority)
{
    ...
    if (painter.isActive())
    {
        ...
        qDebug() << Q_FUNC_INFO << "Couldn't activate painter on buffer. This usually happens because QCustomPlot has "
...
void QCustomPlot::mouseReleaseEvent(QMouseEvent* event)
{
    ...
    if ((mMousePressPos - event->pos()).manhattanLength() < 5) // determine whether it was a click operation
    {
        if (event->button() == Qt::LeftButton)
        {
            ...
            bool additive =
                mInteractions.testFlag(QCP::iMultiSelect) && event->modifiers().testFlag(mMultiSelectModifier);
        ...
        QCPLayerable* clickedLayerable =
            layerableAt(event->pos(), false, &details); // for these signals, selectability is ignored, that's why we
...
void QCustomPlot::drawBackground(QCPPainter* painter)
{
    ...
    if (!mBackgroundPixmap.isNull())
    {
        ...
        {
            painter->drawPixmap(
                mViewport.topLeft(), mBackgroundPixmap, QRect(0, 0, mViewport.width(), mViewport.height()));
        }
    }
}
...
{
    // this method is somewhat similar to toPainter. Change something here, and a change in toPainter might be
    // necessary, too.
    int newWidth, newHeight;
    if (width == 0 || height == 0)
    {
    ...
    if (painter.isActive())
    {
        ...
        {
            if (scale > 1.0) // for scale < 1 we always want cosmetic pens where possible, because else lines might
...
void QCustomPlot::toPainter(QCPPainter* painter, int width, int height)
{
    ...
    if (painter->isActive())
    {
        ...
        setViewport(QRect(0, 0, newWidth, newHeight));
        painter->setMode(QCPPainter::pmNoCaching);
        if (mBackgroundBrush.style() != Qt::NoBrush) // unlike in toPixmap, we can't do QPixmap::fill for
                                                     // Qt::SolidPattern brush style, so we also draw solid fills with
                                                     // fillRect here
            painter->fillRect(mViewport, mBackgroundBrush);
        draw(painter);
        setViewport(oldViewport);
...
bool QCPAxisRect::removeAxis(QCPAxis* axis)
{
    ...
    while (it.hasNext())
    {
        ...
        if (it.value().contains(axis))
        {
            ...
            if (qobject_cast<QCustomPlot*>(parentPlot())) // make sure this isn't called from QObject dtor when
                                                          // QCustomPlot is already destructed (happens when the axis
                                                          // rect is not in any layout and thus QObject-child of
                                                          // QCustomPlot)
...
void QCPAxisRect::drawBackground(QCPPainter* painter)
{
    ...
    if (!mBackgroundPixmap.isNull())
    {
        ...
        {
            painter->drawPixmap(
                mRect.topLeft() + QPoint(0, -1), mBackgroundPixmap, QRect(0, 0, mRect.width(), mRect.height()));
        }
    }
}
...
void QCPAxisRect::updateAxesOffset(QCPAxis::AxisType type)
{
    ...
    bool isFirstVisible = !axesList.first()->visible(); // if the first axis is visible, the second axis (which is where
                                                        // the loop starts) isn't the first visible axis, so initialize
                                                        // with false
...
{
    clearItems();
    if (qobject_cast<QCustomPlot*>(mParentPlot)) // make sure this isn't called from QObject dtor when QCustomPlot is
...
void QCPLegend::selectEvent(QMouseEvent* event, bool additive, const QVariant& details, bool* selectionStateChanged)
{
    ...
    if (details.value<SelectablePart>() == spLegendBox && mSelectableParts.testFlag(spLegendBox))
    {
        ...
        setSelectedParts(additive ? mSelectedParts ^ spLegendBox
                                  : mSelectedParts | spLegendBox); // no need to unset spItems in !additive case,
                                                                   // because they will be deselected by QCustomPlot
                                                                   // (they're normal QCPLayerables with own
                                                                   // deselectEvent)
        if (selectionStateChanged)
            *selectionStateChanged = mSelectedParts != selBefore;
    }
...
void QCPColorScale::setType(QCPAxis::AxisType type)
{
    ...
    if (mType != type)
    {
        ...
        mColorAxis.data()->setScaleLogBase(logBaseTransfer); // scaleType is synchronized among axes in realtime via
                                                             // signals (connected in QCPColorScale ctor), so we only
                                                             // need to take care of log base here
...
void QCPGraph::getPreparedData(QVector<QCPData>* lineData, QVector<QCPData>* scatterData) const
{
        ...
        dataCount >= maxCount) // use adaptive sampling only if there are at least two points per pixel on average
    {
        if (lineData)
        {
            ...
            int reversedRound = keyAxis->rangeReversed() != (keyAxis->orientation() == Qt::Vertical)
                                    ? 1
                                    : 0; // is used to switch between floor (normal) and ceil (reversed) rounding of
            ...
            bool keyEpsilonVariable = keyAxis->scaleType() == QCPAxis::stLogarithmic; // indicates whether keyEpsilon
            ...
            while (it != upperEnd)
            {
                if (it.key() < currentIntervalStartKey + keyEpsilon) // data point is still within same pixel, so skip
                                                                     // it and expand value span of this cluster if
                                                                     // necessary
                {
                ...
                else // new pixel interval started
                {
                    ...
                    {
                        if (lastIntervalEndKey < currentIntervalStartKey - keyEpsilon) // last point is further away, so
                                                                                       // first point of this cluster
                        ...
                        if (it.key() > currentIntervalStartKey + keyEpsilon * 2) // new pixel started further away from
                                                                                 // previous cluster, so make sure the
                                                                                 // last point of the cluster is at a
                                                                                 // real data point
                            lineData->append(
            ...
            if (intervalDataCount >= 2) // last pixel had multiple data points, consolidate them to a cluster
            {
                if (lastIntervalEndKey < currentIntervalStartKey - keyEpsilon) // last point wasn't a cluster, so first
                                                                               // point of this cluster must be at a
                                                                               // real data point
                    lineData->append(
        ...
        if (scatterData)
        {
            ...
            int reversedRound = keyAxis->rangeReversed() ? 1 : 0; // is used to switch between floor (normal) and ceil
            ...
            {
                if (it.key() < currentIntervalStartKey + keyEpsilon) // data point is still within same pixel, so skip
                                                                     // it and expand value span of this pixel if
                                                                     // necessary
                {
                ...
                else // new pixel started
                {
                    ...
                    {
                        // determine value pixel span and add as many points in interval to maintain certain vertical
            ...
            if (intervalDataCount >= 2) // last pixel had multiple data points, consolidate them
            {
                // determine value pixel span and add as many points in interval to maintain certain vertical data
...
const QPolygonF QCPGraph::getChannelFillPolygon(const QVector<QPointF>* lineData) const
{
    ...
    if (mChannelFillGraph.data()->mKeyAxis.data()->orientation() != keyAxis->orientation())
        return QPolygonF(); // don't have same axis orientation, can't fill that (Note: if keyAxis fits, valueAxis will
                            // fit too, because it's always orthogonal to keyAxis)
        ...
        if (scatterData.size() > 0)
        {
            ...
            for (int i = 0; i < scatterData.size(); ++i)
            {
                double currentDistSqr =
                    QVector2D(coordsToPixels(scatterData.at(i).key, scatterData.at(i).value) - pixelPoint)
                        .lengthSquared();
    ...
    else if (inSignDomain == sdNegative) // range may only be in the negative sign domain
    {
        ...
        while (it != mData->constEnd())
        {
            if (!qIsNaN(it.value().value))
            {
                ...
                if (includeErrors) // in case point is in valid sign domain but errobars stretch beyond it, we still
...
void QCPCurve::getCurveData(QVector<QPointF>* lineData) const
{
    ...
    while (it != mData->constEnd())
    {
        ...
        if (currentRegion != prevRegion) // changed region, possibly need to add some optimized edge points or original
                                         // points if entering R
        {
            if (currentRegion != 5) // segment doesn't end in R, so it's a candidate for removal
            {
                ...
                if (prevRegion == 5) // we're coming from R, so add this point optimized
                {
                    lineData->append(getOptimizedPoint(currentRegion,
                                                       ...
                                                       rectRight,
                                                       rectBottom));
                    // in the situations 5->1/7/9/3 the segment may leave R and directly cross through two outer
                    ...
                    *lineData << getOptimizedCornerPoints(prevRegion,
                                                          ...
                                                          rectBottom);
                                                                               ...
                                                                               crossB))
                {
                    // add the two cross points optimized if segment crosses R and if segment isn't virtual zeroth
            ...
            {
                if (it == mData->constBegin()) // it is first point in curve and prevIt is last one. So save optimized
                                               // point for adding it to the lineData in the end
                                                    ...
                                                    double rectBottom) const
{
    ...
    case 1:
    {
        ...
        case 9:
        { // in this case we need another distinction of cases: segment may pass below or above rect, requiring either
          // bottom right or top left corner points
                           ...
                           QPointF& crossB) const
{
    ...
    {
        // one or even zero points found (shouldn't happen unless line perfectly tangent to corner), no need to draw
        // segment
        return false;
    }
...
void QCPColorMap::setColorScale(QCPColorScale* colorScale)
{
    if (mColorScale) // unconnect signals from old color scale
    {
        ...
        disconnect(this,
                   ...
                   SLOT(setDataScaleType(QCPAxis::ScaleType)));
        disconnect(
            this, SIGNAL(gradientChanged(QCPColorGradient)), mColorScale.data(), SLOT(setGradient(QCPColorGradient)));
        disconnect(mColorScale.data(), SIGNAL(dataRangeChanged(QCPRange)), this, SLOT(setDataRange(QCPRange)));
        disconnect(
            mColorScale.data(), SIGNAL(gradientChanged(QCPColorGradient)), this, SLOT(setGradient(QCPColorGradient)));
...
void QCPColorMap::updateMapImage()
{
    ...
    int keyOversamplingFactor =
        mInterpolate ? 1 : (int)(1.0 + 100.0 / (double)keySize); // make mMapImage have at least size 100, factor
    ...
    if (keyAxis->orientation() == Qt::Horizontal)
    {
        ...
        for (int line = 0; line < lineCount; ++line)
        {
            QRgb* pixels = reinterpret_cast<QRgb*>(
                localMapImage->scanLine(lineCount - 1 - line)); // invert scanline index because QImage counts scanlines
...
double QCPItemStraightLine::selectTest(const QPointF& pos, bool onlySelectable, QVariant* details) const
{
    ...
    return distToStraightLine(
        QVector2D(point1->pixelPoint()), QVector2D(point2->pixelPoint() - point1->pixelPoint()), QVector2D(pos));
}
...
QCPItemStraightLine::getRectClippedStraightLine(const QVector2D& base, const QVector2D& vec, const QRect& rect) const
{
    ...
    else // line is skewed
    {
        ...
        {
            // line probably goes through corner of rect, and we got two points there. single out the point pair with
            // greatest distance:
            double distSqrMax = 0;
            QVector2D pv1, pv2;
...
void QCPItemText::draw(QCPPainter* painter)
{
    ...
    QPointF textPos = getTextDrawPoint(
        QPointF(0, 0), textBoxRect, mPositionAlignment); // 0, 0 because the transform does the translation
