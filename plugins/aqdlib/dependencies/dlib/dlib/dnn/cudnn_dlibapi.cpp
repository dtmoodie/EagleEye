...
namespace dlib
{
    ...
    void padded_dot_convolve(tensor& dest,
                                   const tensor& src,
                                   const tensor& filter, const float beta);
    }
    ...
    // ------------------------------------------------------------------------------------
        ...
        {
            DLIB_CASSERT(false && "Not implemented", "Not implemented");
        }
        void batch_normalize_conv_inference (
            const double eps,
            resizable_tensor& dest,
    ...
    // ------------------------------------------------------------------------------------
        ...
        )
        {
            ...
            try
            {
                ...
                if(data.nc() == 1 && data.nr() == 1 &&
                        filters.nc() > 1 && filters.nr() > 1 &&
                        (data.nc() + padding_x * 2 == filters.nc()) &&
                        (data.nr() + padding_y * 2 == filters.nr()))
                {
                    out_num_samples = data.num_samples();
                    out_k   = filters.num_samples();
                    out_nr = 1;
                    out_nc = 1;
                    return;
                }
                ...
                DLIB_CASSERT(status == CUDNN_STATUS_SUCCESS,
                             "Error when calculating workspace size for input of size: "
                             << data.num_samples() << ' ' << data.k() << ' ' <<  data.nr() << ' ' << data.nc() <<
                             "\n with filter of size " << filters.num_samples() << ' ' << filters.k() << ' ' << filters.nr() << ' ' << filters.nc() <<
                             "\n with pad " << padding_y << ' ' << padding_x << " and stride " << stride_y << ' ' << stride_x);
#if CUDNN_VERSION != 2000
        ...
        )
        {
            ...
            if(data.nc() == 1 && data.nr() == 1 &&
                    ...
                    (data.nr() + padding_y * 2 == filters.nr()))
            {
                device::padded_dot_convolve(output, data, filters, beta);
                return;
            }
...
#if CUDNN_VERSION == 2000
            outN = src.num_samples();
            outC = src.k();
            outH = 1 + (src.nr() + 2 * padding_y - window_height) / stride_y;
            outW = 1 + (src.nc() + 2 * padding_x - window_width ) / stride_x;
#else
