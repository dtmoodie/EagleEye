...
namespace EagleLib
{
    ...
    template<typename T> bool qualifier(const EagleLib::Parameter::Ptr& param, int width = -1, int height = -1, int channels = -1, int type = -1)
    {
    ...
    {
    public:
        static boost::function<bool(const EagleLib::Parameter::Ptr&)> get(int width = -1, int height = -1, int channels = -1, int type = -1)
        {
