...
namespace EagleLib
{
	...
	public:
		...
		void RegisterNodeInfo(const char* nodeName, std::vector<NodeType>& nodeInfo);
		std::vector<NodeType>& GetNodeInfo(std::string& nodeName);
	...
	private:
		...
		std::map<std::string,std::vector<NodeType>>					m_nodeInfoMap;
