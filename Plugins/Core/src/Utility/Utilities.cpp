...
{
    updateParameter<boost::function<void(void)>>("Call all input functions", boost::bind(&SyncFunctionCall::call,
this));
    if(firstInit)
    {
...
cv::cuda::GpuMat SyncFunctionCall::doProcess(cv::cuda::GpuMat &img, cv::cuda::Stream &stream)
{
    ...
    if(full == true)
    {
        addInputParam<boost::function<void(void)>>("Input " +
boost::lexical_cast<std::string>(_parameters.size()))->SetQualifier(boost::bind(&functionQualifier, _1));
    }
    return img;
}
...
bool RegionOfInterest::processImpl()
{
    if (roi.area())
    {
        ...
        ROI_param.updateData(SyncedMemory(h_mats, d_mats, state),
                             image_param.getTimestamp(),
                             _ctx.get(),
                             image_param.getCoordinateSystem(),
                             image_param.getFrameNumber());
        return true;
    }
    return false;
}
