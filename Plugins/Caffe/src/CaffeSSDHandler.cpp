...
void SSDHandler::handleOutput(const caffe::Net<float>& net,
                              ...
                              const std::vector<aq::DetectedObject2d>& objs)
{
    ...
    for (size_t i = 0; i < static_cast<size_t>(num_detections); ++i)
    {
        if ((detection_threshold.size() == 1 && confidence[static_cast<int>(i)][0] > detection_threshold[0]) ||
            (labels[static_cast<int>(i)][0] < detection_threshold.size() &&
             confidence[static_cast<int>(i)][0] >
                 detection_threshold[static_cast<size_t>(labels[static_cast<int>(i)][0])]))
        {
            ...
            obj.bounding_box.width =
                (xmax[static_cast<int>(i)][0] - xmin[static_cast<int>(i)][0]) * bounding_boxes[num].width;
            obj.bounding_box.height =
                (ymax[static_cast<int>(i)][0] - ymin[static_cast<int>(i)][0]) * bounding_boxes[num].height;
            ...
            if (this->labels && labels[static_cast<int>(i)][0] < this->labels->size())
                obj.classification = Classification((*this->labels)[size_t(labels[static_cast<int>(i)][0])],
                                                    confidence[static_cast<int>(i)][0],
                                                    int(labels[static_cast<int>(i)][0]));
            else
                obj.classification =
                    Classification("", confidence[static_cast<int>(i)][0], int(labels[static_cast<int>(i)][0]));
    ...
    detections_param.updateData(objects, mo::tag::_param = input_param, _ctx);
}
