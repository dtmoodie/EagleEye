...
void FCNHandler::handleOutput(const caffe::Net<float>& net,
...
void FCNSingleClassHandler::handleOutput(const caffe::Net<float>& net,
                                         const std::vector<cv::Rect>& bounding_boxes,
                                         mo::ITParam<aq::SyncedMemory>& input_param,
                                         const std::vector<aq::DetectedObject2d>& objs)
{
    ...
    {
        blob->gpu_data();
        auto wrapped = aq::nodes::CaffeImageClassifier::WrapBlob(*blob);
